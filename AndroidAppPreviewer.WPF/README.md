...
Desktop-host для native preview-plugin. Решение Visual Studio: `AndroidAppPreviewer.sln`; собирать нужно в `Debug|x64` или `Release|x64`. DLL приложения выбирается системным диалогом кнопки «Выбрать приложение» или параметром `--plugin C:\path\Application.PreviewPlugin.dll`. Плагин через `xp_get_plugin_info` обязан передать расположение package-ресурсов, исходного XAML, стартовой страницы и каталога контролов; Previewer не использует сохранённый путь к исходникам как fallback.
